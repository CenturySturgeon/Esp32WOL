...
sequenceDiagram
    ...
    A->>A: Retrieve salt from NVS
    A->>A: PBKDF2-HMAC-SHA256(password, salt, 100k iter)
    A->>A: Constant-time compare vs stored hash
    alt Password valid
...
| `user_{N}_salt`                           | string | Random 16-byte salt for PBKDF2 (hex, 32 chars)             |
| `user_{N}_hash`                           | string | PBKDF2-HMAC-SHA256 hash of password + salt (hex, 64 chars) |
...
- **Password hashing**: Salted PBKDF2-HMAC-SHA256 (100k iterations) verified via mbedTLS with constant-time comparison. Mitigates offline brute-force and rainbow table attacks while maintaining ESP32 performance.
...
| `mbedtls`          | PBKDF2 password verification (`pkcs5.h`), HMAC for TOTP, TLS/Cert management |
...
> *Note: `credentialsFabricator.py` now generates a cryptographically random 16-byte salt per user and computes PBKDF2-HMAC-SHA256 hashes (100k iterations) before writing to `secrets.csv`. Existing users must regenerate credentials after firmware updates.*
